...
const main = async () => {
    ...
    const ethBal = await impersonatedSigner.provider.getBalance(impersonatedSigner.address);
    ...
    const ethBal2 = await impersonatedSigner.provider.getBalance(impersonatedSigner.address);
    ...
    console.log("eth balance after swap", ethers.formatUnits(ethBal2, 18));
}
